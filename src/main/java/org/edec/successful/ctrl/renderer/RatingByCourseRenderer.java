...
import org.edec.successful.model.GroupModel;
import org.edec.successful.model.StudentModel;
import org.edec.successful.model.SubjectModel;
...
public class RatingByCourseRenderer implements ListitemRenderer<CourseModel> {
    ...
    public RatingByCourseRenderer (List<CourseModel> courseModels) {
        this.courseModels = courseModels;
    }

    public RatingByCourseRenderer () {

    }

    @Override
    public void render (Listitem li, CourseModel data, int index) throws Exception {
        ...
        //Курс
        Listcell listCellCourse = new Listcell(String.valueOf(data.getCourse()));
        listCellCourse.setParent(li);
        listCellCourse.addEventListener(Events.ON_CLICK, (EventListener<MouseEvent>) event -> {
            List<GroupModel> groups = ((CourseModel) ((Listitem) (event.getTarget().getParent())).getValue()).getGroups();
            Listbox par = ((Listbox) li.getParent());
            RatingByGroupRenderer ratingByGroupRenderer = new RatingByGroupRenderer(courseModels, null);
        ...
        allFullLC.addEventListener(
                Events.ON_CLICK, (EventListener<MouseEvent>) event -> DialogUtil.info(allFullStudentsLocal.size() + "", "FYI"));
        //5
    ...
    public void getAll (CourseModel data) {

...
public class RatingByGroupRenderer implements ListitemRenderer<GroupModel> {
    ...
    public RatingByGroupRenderer (List<CourseModel> courseModels, List<DepartmentModel> departmentModel) {
    ...
    public void render (Listitem li, GroupModel data, int index) throws Exception {
        ...
        //#
        Listcell listCellBack = new Listcell("<<");
        listCellBack.setStyle("background:#bfbfbf");
        listCellBack.setParent(li);
        listCellBack.addEventListener(Events.ON_CLICK, (EventListener<MouseEvent>) event -> {
            Listbox par = ((Listbox) li.getParent());
            if (courseModels != null) {
                RatingByCourseRenderer ratingByCourseRenderer = new RatingByCourseRenderer(courseModels);
                par.setItemRenderer(ratingByCourseRenderer);
                par.setModel(new ListModelList<>(courseModels));
                par.renderAll();
                currentFooter.getChildren().clear();
                ratingByCourseRenderer.calcFooter(currentFooter);
            }

            if (departmentModel != null) {
                RatingByTChairRenderer ratingByTChairRenderer = new RatingByTChairRenderer(departmentModel);
                par.setItemRenderer(ratingByTChairRenderer);
                par.setModel(new ListModelList<>(departmentModel));
                par.renderAll();
                currentFooter.getChildren().clear();
                ratingByTChairRenderer.calcFooter(currentFooter);
            }
        });
        //Группа
        Listcell listCellCourse = new Listcell(String.valueOf(data.getGroupname()));
        listCellCourse.setParent(li);
        //Кол-во людей
        Listcell allStudentLC = new Listcell(String.valueOf(data.getStudents().size()));
        allStudentLC.setParent(li);
        final List<StudentModel> allStudentsLocal = data.getStudents();
        allStudentLC.addEventListener(Events.ON_CLICK, (EventListener<MouseEvent>) event -> {
            showStudents(allStudentsLocal);
        });
        //Сдали
        Integer percent = allFullStudents.size() * 100 / data.getStudents().size();
        ...
        allFullLC.addEventListener(Events.ON_CLICK, (EventListener<MouseEvent>) event -> {
            showStudents(allFullStudentsLocal);
        });
        //5
        ...
        //Есть долги
        Integer percentMiss = missFullStudents.size() * 100 / data.getStudents().size();
        ...
        //Не сдана вся сессия
        Integer percentMissFull = missFullStudents.size() * 100 / data.getStudents().size();
        ...
        totalCountStudent += data.getStudents().size();
    ...
    public void getAll (GroupModel data) {
        for (StudentModel studentModel : data.getStudents()) {
            ...
            for (SubjectModel subjectModel : studentModel.getSubjects()) {
                ...
                if (maxRating == RatingConst.UNSATISFACTORILY.getRating() ||
                    subjectModel.getLastRating().getRating() == RatingConst.NOT_PASS.getRating()) {
                    two++;
                }
    ...
    public void calcFooter (Listfoot lfStudent) {
        ...
        Integer percent = totalCountallFullStudents * 100 / totalCountStudent;
